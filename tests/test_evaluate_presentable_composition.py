...
def test_evaluate_presentable_produces_scores(tmp_path: Path, monkeypatch) -> None:
    ...
    assert 0.0 <= float(payload["presentability_score"]) <= 1.0
    assert 0.0 <= float(payload["ratio_compliance_score"]) <= 1.0
