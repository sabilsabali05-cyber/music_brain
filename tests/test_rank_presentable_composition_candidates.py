...
def test_rank_candidates_selects_and_copies_package(tmp_path: Path, monkeypatch) -> None:
    ...
    ranking = analyzer.rank_candidates()
    assert ranking["selected_candidate"].startswith("candidate_")
    assert (tmp_path / "outputs" / analyzer.PROJECT_ID / "selected" / "full.mid").exists()
    assert (tmp_path / "outputs" / analyzer.PROJECT_ID / "selected" / "stems").exists()
