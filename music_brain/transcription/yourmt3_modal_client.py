...
class YourMT3ModalTranscriber(BaseTranscriber):
    def __init__(self, endpoint: str | None, remote_call: RemoteCall | None = None) -> None:
    ...
    def _invoke_modal(self, normalized_audio_bytes: bytes) -> dict[str, object]:
        ...
        try:
            ...
            raise RuntimeError(
                "modal package is required for backend=modal. Install dependencies and run `modal setup`."
            ) from exc

        cls = modal.Cls.from_name("music-brain-v2", "YourMT3ModalRunner")
        runner = cls()
        return runner.transcribe.remote(normalized_audio_bytes)
    ...
    def transcribe(self, request: TranscriptionRequest) -> TranscriptionResult:
        ...
        if not isinstance(midi_bytes_raw, (bytes, bytearray)):
            raise RuntimeError("Modal YourMT3 response is missing valid midi_bytes")
        midi_bytes = bytes(midi_bytes_raw)

        provider_used = str(payload.get("provider_used", "none"))
        backend = str(payload.get("backend", "none"))
        model_version = str(payload.get("model_version", ""))
        if provider_used != "yourmt3" or backend != "modal":
            raise RuntimeError(
                "Modal YourMT3 backend returned unexpected identity metadata: "
                f"provider_used={provider_used}, backend={backend}"
            )
        if not model_version:
            raise RuntimeError("Modal YourMT3 response is missing model_version")
