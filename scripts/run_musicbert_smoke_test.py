...
def _write_smoke_report(output_dir: Path, payload: dict) -> tuple[Path, Path]:
    ...
    json_path = output_dir / "musicbert_smoke_result.json"
    md_path = output_dir / "musicbert_smoke_result.md"
...
def run_musicbert_smoke_test() -> dict:
    ...
    try:
        ...
        probe = torch.tensor([[1.0, 2.0], [3.0, 4.0]]).mean()
        artifact_path = ROOT_DIR / "reports" / "model_integrations" / "musicbert_smoke_artifact.json"
        provenance_path = ROOT_DIR / "reports" / "model_integrations" / "musicbert_smoke_provenance.json"
        artifact_path.parent.mkdir(parents=True, exist_ok=True)
        artifact_path.write_text(
            json.dumps({"backend": "musicbert", "probe_mean": float(probe.item())}, indent=2, ensure_ascii=True) + "\n",
            encoding="utf-8",
        )
...
def main() -> int:
    ...
    print(f"MUSICBERT_ENABLED={payload['musicbert_enabled']}")
