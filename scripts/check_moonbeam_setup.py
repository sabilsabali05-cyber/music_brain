...
def _read_moonbeam_config() -> tuple[dict[str, Any], bool]:
...
def evaluate_moonbeam_setup() -> dict[str, Any]:
    settings, using_local_config = _read_moonbeam_config()
    ...
    moonbeam_configured = using_local_config and enabled and repo_ok and model_ok and tokenizer_ok
    ...
    if not using_local_config:
        ...
        next_setup_step = (
            ...
            "config/model_integrations/model_integrations.local.json, then set models.moonbeam.enabled=true."
        )
    elif not enabled:
        ...
        next_setup_step = "Set models.moonbeam.repo_path to an existing local Moonbeam repository path."
    elif not model_ok:
        unavailable_reason = "model_path_missing"
        next_setup_step = "Set models.moonbeam.model_path to an existing local model weights path."
    elif not tokenizer_ok:
        unavailable_reason = "tokenizer_path_missing"
        next_setup_step = "Set models.moonbeam.tokenizer_path to an existing local tokenizer path."
    elif not smoke_test_enabled:
        unavailable_reason = "smoke_test_disabled_in_config"
        next_setup_step = "Set models.moonbeam.smoke_test_enabled=true to allow smoke validation."
    else:
        unavailable_reason = "ready_for_smoke_test"
        next_setup_step = "Run scripts/dev.cmd run-moonbeam-smoke-test to validate runtime import smoke test."
    ...
    return {
        ...
        "moonbeam_enabled": enabled if using_local_config else False,
        "moonbeam_configured": moonbeam_configured,
        ...
        "limitations": [
            "This checker does not import heavyweight Moonbeam packages.",
...
def write_moonbeam_setup_report(output_dir: Path) -> tuple[Path, Path, dict[str, Any]]:
    payload = evaluate_moonbeam_setup()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check Moonbeam local setup readiness without heavy imports.")
    parser.add_argument("--output-dir", default=REPORT_DIR.as_posix())
    args = parser.parse_args()
    output_dir = Path(args.output_dir)
    if not output_dir.is_absolute():
        output_dir = ROOT_DIR / output_dir
    json_path, md_path, payload = write_moonbeam_setup_report(output_dir)
    print(f"MOONBEAM_SETUP_STATUS_JSON={json_path.as_posix()}")
    print(f"MOONBEAM_SETUP_STATUS_MD={md_path.as_posix()}")
