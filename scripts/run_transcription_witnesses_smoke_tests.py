from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path
from typing import Any

ROOT_DIR = Path(__file__).resolve().parent.parent
if str(ROOT_DIR) not in sys.path:
    sys.path.insert(0, str(ROOT_DIR))

from scripts.check_transcription_witnesses_setup import evaluate_transcription_witnesses_setup


def run_transcription_witnesses_smoke_tests() -> dict[str, Any]:
    base = evaluate_transcription_witnesses_setup()
    return {
        **base,
        "status": "unavailable_safe",
        "smoke_test_passed": False,
        "smoke_tests_performed": [],
        "transcription_outputs_generated": [],
        "smoke_test_notes": [
            "Smoke tests validate setup semantics only.",
            "No audio processing, no transcription execution, and no downloads were performed.",
            "Witness scaffolds explicitly prevent fake transcription outputs.",
        ],
    }


def write_smoke_report(output_dir: Path) -> tuple[Path, Path, dict[str, Any]]:
    payload = run_transcription_witnesses_smoke_tests()
    output_dir.mkdir(parents=True, exist_ok=True)
    json_path = output_dir / "transcription_witnesses_setup_status.json"
    md_path = output_dir / "transcription_witnesses_setup_status.md"
    json_path.write_text(json.dumps(payload, indent=2, ensure_ascii=True) + "\n", encoding="utf-8")
    lines = [
        "# Transcription Witnesses Smoke Test Status",
        "",
        f"- status: `{payload['status']}`",
        f"- smoke_test_passed: `{payload['smoke_test_passed']}`",
        f"- yourmt3_available: `{payload['yourmt3_available']}`",
        f"- basic_pitch_available: `{payload['basic_pitch_available']}`",
        f"- transcription_performed: `{payload['transcription_performed']}`",
        f"- witness_policy: `{payload['witness_policy']}`",
        "- model_training_has_occurred: `False`",
        "- audio_processing_performed: `False`",
        "- downloads_performed: `False`",
    ]
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return json_path, md_path, payload


def main() -> int:
    parser = argparse.ArgumentParser(description="Run transcription witness smoke scaffolds without transcription.")
    parser.add_argument("--output-dir", default="reports/model_integrations")
    args = parser.parse_args()
    output_dir = Path(args.output_dir)
    if not output_dir.is_absolute():
        output_dir = ROOT_DIR / output_dir
    json_path, md_path, payload = write_smoke_report(output_dir)
    print(f"TRANSCRIPTION_WITNESSES_SMOKE_JSON={json_path.as_posix()}")
    print(f"TRANSCRIPTION_WITNESSES_SMOKE_MD={md_path.as_posix()}")
    print(f"SMOKE_TEST_PASSED={payload['smoke_test_passed']}")
    print("YOURMT3_AVAILABLE=False")
    print("BASIC_PITCH_AVAILABLE=False")
    print("TRANSCRIPTION_PERFORMED=False")
    print("MODEL_TRAINING_HAS_OCCURRED=False")
    print("WITNESS_POLICY=witness_not_truth")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
