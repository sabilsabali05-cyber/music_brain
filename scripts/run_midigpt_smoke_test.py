...
def run_midigpt_smoke_test() -> dict:
    ...
    if unavailable_reason in {"disabled_or_missing_local_config", "disabled_in_local_config"}:
        return {
            **base,
            ...
            "next_setup_step": (
                "Enable MIDI-GPT and set repo_path/model_path/tokenizer_path in "
    ...
    return {
        **base,
        ...
        "unavailable_reason": "",
        "next_setup_step": "MIDI-GPT smoke test passed. Keep generation hooks unavailable until full inference wiring is complete.",
        "model_training_has_occurred": False,
        "smoke_test_notes": [
            "Minimal runtime dependency probe passed.",
            "No generation output was synthesized by this smoke test.",
            "No weights downloaded and no training performed.",
        ],
    }
