...
def extract_rhythm_features(performance_manifest_path: Path, *, output_dir: Path | None = None) -> Path:
    ...
    for source in midi_sources:
        ...
        feature_values = rhythm_features_from_events(events)
        confidence = 0.9 if source.kind == "merged" else 0.7
        ...
        if feature_values.get("median_ioi_seconds", 0.0) == 0.0:
...
def main() -> int:
    ...
    output_path = extract_rhythm_features(Path(args.performance_manifest), output_dir=output_dir)
