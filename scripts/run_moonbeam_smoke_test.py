...
def _write_smoke_report(output_dir: Path, payload: dict) -> tuple[Path, Path]:
    ...
    json_path = output_dir / "moonbeam_smoke_result.json"
    md_path = output_dir / "moonbeam_smoke_result.md"
...
def run_moonbeam_smoke_test() -> dict:
    ...
    try:
        ...
        probe = torch.tensor([0.5, 1.5, 2.5]).sum()
        artifact_path = ROOT_DIR / "reports" / "model_integrations" / "moonbeam_smoke_artifact.json"
        provenance_path = ROOT_DIR / "reports" / "model_integrations" / "moonbeam_smoke_provenance.json"
        artifact_path.parent.mkdir(parents=True, exist_ok=True)
        artifact_path.write_text(
            json.dumps({"backend": "moonbeam", "probe_sum": float(probe.item())}, indent=2, ensure_ascii=True) + "\n",
            encoding="utf-8",
        )
        provenance_path.write_text(
            json.dumps(
                {
                    "backend": "moonbeam",
                    "probe_type": "torch_tensor_reduction",
...
def main() -> int:
    ...
    print(f"MOONBEAM_ENABLED={payload['moonbeam_enabled']}")
