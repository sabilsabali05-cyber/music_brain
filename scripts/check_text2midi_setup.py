...
def _read_text2midi_config() -> tuple[dict[str, Any], bool]:
...
def evaluate_text2midi_setup() -> dict[str, Any]:
    settings, using_local_config = _read_text2midi_config()
    ...
    text2midi_configured = using_local_config and enabled and repo_ok and model_ok
    ...
    if not using_local_config:
        ...
        next_setup_step = (
            ...
            "config/model_integrations/model_integrations.local.json, then set models.text2midi.enabled=true."
        )
    elif not enabled:
        ...
        next_setup_step = "Set models.text2midi.repo_path to an existing local Text2MIDI repository path."
    elif not model_ok:
        unavailable_reason = "model_path_missing"
        next_setup_step = "Set models.text2midi.model_path to an existing local model path."
    elif not smoke_test_enabled:
        unavailable_reason = "smoke_test_disabled_in_config"
        next_setup_step = "Set models.text2midi.smoke_test_enabled=true to allow local smoke validation."
    else:
        unavailable_reason = "ready_for_smoke_test"
        next_setup_step = "Run scripts/dev.cmd run-text2midi-smoke-test to validate local Text2MIDI setup."
    ...
    return {
        ...
        "text2midi_enabled": enabled if using_local_config else False,
        "text2midi_configured": text2midi_configured,
        ...
        "limitations": [
            "No heavyweight Text2MIDI package imports are attempted unless local config is enabled.",
...
def write_text2midi_setup_report(output_dir: Path) -> tuple[Path, Path, dict[str, Any]]:
    payload = evaluate_text2midi_setup()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check Text2MIDI local setup readiness without heavy imports.")
    parser.add_argument("--output-dir", default=REPORT_DIR.as_posix())
    args = parser.parse_args()
    output_dir = Path(args.output_dir)
    if not output_dir.is_absolute():
        output_dir = ROOT_DIR / output_dir
    json_path, md_path, payload = write_text2midi_setup_report(output_dir)
    print(f"TEXT2MIDI_SETUP_STATUS_JSON={json_path.as_posix()}")
    print(f"TEXT2MIDI_SETUP_STATUS_MD={md_path.as_posix()}")
