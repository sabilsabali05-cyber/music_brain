...
def describe_task() -> dict[str, Any]:
    return {"task_type": "mert_embedding", "model_id": "mert", "evidence_policy": "embeddings=semantic_evidence_not_truth"}


def estimate_cost(input_payload: dict[str, Any]) -> float:
    return round(float(input_payload.get("segment_count", 1)) * 0.012, 4)
...
def plan_job(input_payload: dict[str, Any]) -> dict[str, Any]:
    return {"task_type": "mert_embedding", "estimated_cost_usd": estimate_cost(input_payload), "planned_only": True}
