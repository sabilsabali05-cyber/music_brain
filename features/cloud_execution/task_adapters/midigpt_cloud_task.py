...
def describe_task() -> dict[str, Any]:
    return {
        "task_type": "midigpt_symbolic_generation",
        "model_id": "midigpt",
...
def estimate_cost(input_payload: dict[str, Any]) -> float:
    return round(float(input_payload.get("prompt_count", 1)) * 0.025, 4)
...
def plan_job(input_payload: dict[str, Any]) -> dict[str, Any]:
    return {"task_type": "midigpt_symbolic_generation", "estimated_cost_usd": estimate_cost(input_payload), "planned_only": True}
