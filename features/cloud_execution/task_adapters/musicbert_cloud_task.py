...
def describe_task() -> dict[str, Any]:
    return {
        "task_type": "musicbert_ranking",
        "model_id": "musicbert",
        "evidence_policy": "ranking=assistive_signal_not_truth",
    }


def estimate_cost(input_payload: dict[str, Any]) -> float:
    return round(float(input_payload.get("candidate_count", 1)) * 0.008, 4)
...
def plan_job(input_payload: dict[str, Any]) -> dict[str, Any]:
    return {"task_type": "musicbert_ranking", "estimated_cost_usd": estimate_cost(input_payload), "planned_only": True}
